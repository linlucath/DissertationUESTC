...
**注意：**使用`\tablenotetext`命令的前提是对表格首列进行特定设置，用户**必须**通过`p{<长度>}`或`m{<长度>}`来人为指定`longtable`的首列宽度，**切不可**将之设置为`c`、`r`或`l`，否则第三项参数将失去原有的作用，导致怪异的排版结果。另外，表格中最后一条`\tablenotetext`之后不需要`\\`，否则表尾与下方文本的间隔将偏大。
